...
def data_to_db(df):
    ...
    unit_info = df[['beds', 'baths', 'area']].values.tolist()
    ...
    for i, house_unit in enumerate(house_units):
        cursor.execute(q1, house_unit)
        last_id = cursor.lastrowid
        cursor.execute(q2, [last_id] + unit_images[i])
        cursor.execute(q3, [last_id] + unit_links[i])
        cursor.execute(q4, [last_id] + unit_addresses[i])
        cursor.execute(q5, [last_id] + unit_info[i])
    conn.commit()
